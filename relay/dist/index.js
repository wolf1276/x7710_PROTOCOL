...
const scval_builder_1 = require("./stellar/scval-builder");
...
const addressesPath = path_1.default.join(__dirname, '../contract-addresses.json');
...
async function signAndSubmitTx(tx, signers) {
...
function parseTxEvents(resultMeta) {
    // Decode meta to extract contract events
    try {
        const meta = typeof resultMeta === 'string'
            ? stellar_sdk_1.xdr.TransactionMeta.fromXDR(resultMeta, 'base64')
            : resultMeta;
        const v3 = meta.v3();
        if (!v3)
            return [];
        ...
        v3.sorobanMeta()?.events().forEach((evt) => {
            try {
                ...
                const topics = evt.body().v0().topics().map(t => (0, stellar_sdk_1.scValToNative)(t));
                const data = (0, stellar_sdk_1.scValToNative)(evt.body().v0().data());
...
app.post('/api/register', async (req, res) => {
    ...
    try {
        const ownerKeypair = stellar_sdk_1.Keypair.fromSecret(ownerSecret);
        const ownerPubkey = ownerKeypair.publicKey();
        const sourceAccount = await getAccount(ownerPubkey);
        const contract = new stellar_sdk_1.Contract(contractAddresses.registry);
        const tx = new stellar_sdk_1.TransactionBuilder(sourceAccount, {
            fee: stellar_sdk_1.BASE_FEE,
            networkPassphrase: NETWORK_PASSPHRASE,
        })
            .addOperation(contract.call('register_account', new stellar_sdk_1.Address(ownerPubkey).toScVal(), new stellar_sdk_1.Address(policyAddress).toScVal(), (0, stellar_sdk_1.nativeToScVal)(agents.map(a => new stellar_sdk_1.Address(a)))))
            .setTimeout(30)
            .build();
...
app.post('/api/set-policy', async (req, res) => {
    ...
    try {
        ...
        const ownerPubkey = ownerKeypair.publicKey();
        const sourceAccount = await getAccount(ownerPubkey);
        const contract = new stellar_sdk_1.Contract(contractAddresses.policy);
        // Construct the PolicyConfig contracttype struct
        const configStruct = {
            owner: new stellar_sdk_1.Address(ownerPubkey),
            max_spend_per_action: (0, scval_builder_1.humanToStroops)(maxSpendPerAction),
            max_daily_spend: (0, scval_builder_1.humanToStroops)(maxDailySpend),
            daily_spent: 0n,
            daily_reset_ledger: 0,
            allowed_assets: allowedAssets.map((asset) => Symbol.prototype), // Wait, allowed_assets is Vec<Symbol>
        ...
        const assetSymbols = allowedAssets.map((asset) => stellar_sdk_1.xdr.ScVal.scvSymbol(asset));
        const tx = new stellar_sdk_1.TransactionBuilder(sourceAccount, {
            fee: stellar_sdk_1.BASE_FEE,
            networkPassphrase: NETWORK_PASSPHRASE,
        })
            .addOperation(contract.call('set_policy', new stellar_sdk_1.Address(ownerPubkey).toScVal(), (0, stellar_sdk_1.nativeToScVal)({
            owner: new stellar_sdk_1.Address(ownerPubkey),
            max_spend_per_action: (0, scval_builder_1.humanToStroops)(maxSpendPerAction),
            max_daily_spend: (0, scval_builder_1.humanToStroops)(maxDailySpend),
            daily_spent: 0n,
...
app.post('/api/execute-action', async (req, res) => {
    ...
    try {
        ...
        const routerContract = new stellar_sdk_1.Contract(contractAddresses.router);
        const actionRequest = {
            owner: ownerAddress,
            agent: agentAddress,
        ...
        const actionRequestScVal = (0, scval_builder_1.buildActionRequestScVal)(actionRequest, ownerAddress);
        const tx = new stellar_sdk_1.TransactionBuilder(sourceAccount, {
            fee: stellar_sdk_1.BASE_FEE,
            networkPassphrase: NETWORK_PASSPHRASE,
        })
            .addOperation(routerContract.call('execute_action', new stellar_sdk_1.Address(contractAddresses.registry).toScVal(), actionRequestScVal))
            .setTimeout(30)
            .build();
        const result = await signAndSubmitTx(tx, [ownerKeypair]);
        res.json({ success: true, ...result });
...
app.get('/api/registry/:owner/agents', async (req, res) => {
    ...
    try {
        ...
        const dummyKeypair = stellar_sdk_1.Keypair.random();
        const dummyAccount = new stellar_sdk_1.Account(dummyKeypair.publicKey(), '0');
        const tx = new stellar_sdk_1.TransactionBuilder(dummyAccount, {
            fee: stellar_sdk_1.BASE_FEE,
            networkPassphrase: NETWORK_PASSPHRASE,
        })
            .addOperation(contract.call('list_agents', new stellar_sdk_1.Address(owner).toScVal()))
            .setTimeout(30)
            .build();
        const simRes = await server.simulateTransaction(tx);
        if (!stellar_sdk_1.rpc.Api.isSimulationSuccess(simRes)) {
            return res.status(400).json({ error: 'Simulation failed: ' + JSON.stringify(simRes) });
        }
        const result = (0, stellar_sdk_1.scValToNative)(simRes.result.retval);
...
app.get('/api/registry/:owner/policy-contract', async (req, res) => {
    ...
    try {
        ...
        const tx = new stellar_sdk_1.TransactionBuilder(dummyAccount, {
            ...
            .addOperation(contract.call('get_policy_contract', new stellar_sdk_1.Address(owner).toScVal()))
            .setTimeout(30)
            .build();
        const simRes = await server.simulateTransaction(tx);
        if (!stellar_sdk_1.rpc.Api.isSimulationSuccess(simRes)) {
...
app.get('/api/registry/:owner/is-authorized/:agent', async (req, res) => {
    ...
    try {
        ...
        const tx = new stellar_sdk_1.TransactionBuilder(dummyAccount, {
            ...
            .addOperation(contract.call('is_authorized_agent', new stellar_sdk_1.Address(owner).toScVal(), new stellar_sdk_1.Address(agent).toScVal()))
            .setTimeout(30)
            .build();
        const simRes = await server.simulateTransaction(tx);
        if (!stellar_sdk_1.rpc.Api.isSimulationSuccess(simRes)) {
...
app.get('/api/events', async (req, res) => {
    try {
        ...
        const parsedEvents = response.events.map((evt) => {
            ...
            let data = null;
            try {
                topics = evt.topic.map((t) => (0, stellar_sdk_1.scValToNative)(t));
                data = (0, stellar_sdk_1.scValToNative)(evt.value);
            }
            catch (e) {
